...
\f\r"'\`<>=]`;
  ...
  var blockedUsers_default = new class {
    constructor() {
      ...
      __publicField(this, "add", ({ id, name }) => {
        if (this.has(id)) {
          return;
        }
  ...
  var _SettingsDrawer = class _SettingsDrawer {
    constructor() {
      ...
      // 添加官方同步黑名单的函数
      __publicField(this, "addAllToBlacklist", () => __async(this, null, function* () {
        const allUserIDs = blockedUsers_default.distinctID();
        if (allUserIDs.length === 0) {
          console.log("本地没有存储屏蔽用户。");
          return;
        }
        let officialBlacklistIDs = [];
        try {
          const resp = yield fetch("https://api.bilibili.com/x/relation/blacks", {
            method: "GET",
            credentials: "include"
          });
          ...
          officialBlacklistIDs = result.data.list.map(
            (item) => String(item.mid)
          );
        } catch (err) {
        ...
        for (const id of toAdd) {
          try {
            ...
            const res = yield fetch("https://api.bilibili.com/x/relation/modify", {
              method: "POST",
              credentials: "include",
              headers: {
